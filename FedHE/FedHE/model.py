...
class model(nn.Module):
    def __init__(self, embed_size):
        ...
        self.GAT_item_neighbor = GraphAttentionLayer(embed_size, embed_size)
        self.relation_item_neighbor = nn.Parameter(torch.randn(embed_size))
        self.relation_item_self = nn.Parameter(torch.randn(embed_size))
        self.d = nn.Parameter(torch.randn(2 * embed_size))

    def predict(self, user_embedding, item_embedding):
        return torch.matmul(user_embedding, item_embedding.t())


    def forward(self, feature_self, feature_neighbor, feature_item, is_item):
        if not is_item:
            ...
            f_n = self.GAT_item_neighbor(feature_self, feature_neighbor)
            e_n = torch.matmul(self.d, torch.cat((f_n, self.relation_item_neighbor)))
            e_s = torch.matmul(self.d, torch.cat((feature_self, self.relation_item_self)))
            m = nn.Softmax(dim=-1)
            e_tensor = torch.stack([e_n, e_s])
            e_tensor = m(e_tensor)
            r_n, r_s = e_tensor
            item_embedding = r_s * feature_self + r_n * f_n
